  ...
  Object.keys(formValues).forEach((key) => {
    ...
    if (!value || typeof value === "undefined") {
        ...
        if (value.length < 8) {
            ...
            valid = false;
            errors[key] = `${messageKey[key]} must contain only letters and numbers`;
        } else if (key === "password" && !validatePassword(value)) {
            valid = false;
            errors[key] = `${messageKey[key]} must contain at least one uppercase letter, one lowercase letter, and one number`;
        }
